...
//========== Type-level Recursive-descent JSON Parser ==========//
...
type EvalString<T, S extends string = ''>
  ...
  : (T extends `\\${infer C}${infer U}` ? C extends keyof Escapes ? [C, U] : false : false) extends [infer C, infer U] 
            ? EvalString<U, `${S}${C extends keyof Escapes ? Escapes[C] : never}`> : T extends `${infer C}${infer U}` ? EvalString<U, `${S}${C}`>
  : false
...
type Json1 = Parse<'{ "asdf": ["1", "2", "3"] }'>;
type Json2 = Parse<'["hello", true, false, "world", null]'>;